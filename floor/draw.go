...
type BlocAnimated struct {
	stepAnimation  int
	frameAnimation int
}

var animationStruct []BlocAnimated = []BlocAnimated{BlocAnimated{0, 0}, BlocAnimated{0, 0}, BlocAnimated{0, 0}, BlocAnimated{0, 0}, BlocAnimated{0, 0}, BlocAnimated{0, 0}}
	...
	for y := range f.content {
		for x := range f.content[y] {
			if f.content[y][x] != -1 {
				...
				shiftY := animationStruct[f.content[y][x]].stepAnimation * configuration.Global.TileSize

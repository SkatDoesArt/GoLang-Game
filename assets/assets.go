...
//go:embed portail.png
var portalBytes []byte
var PortalImage *ebiten.Image

//go:embed tresor.png
var tresorBytes []byte
var TresorImage *ebiten.Image
...
func Load() {
	...
	// ajouté pour portal
	decoded, _, err = image.Decode(bytes.NewReader(portalBytes))
	if err != nil {
		log.Fatal(err)
	}
	PortalImage = ebiten.NewImageFromImage(decoded)

	// ajouté pour tresor
	decoded, _, err = image.Decode(bytes.NewReader(tresorBytes))
	if err != nil {
		log.Fatal(err)
	}
	TresorImage = ebiten.NewImageFromImage(decoded)

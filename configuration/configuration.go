...
type Configuration struct {
	...
	NumTileForDebug               int
	CameraMode                    int //nouveau mode camera bloquante en mettant '2' dans la configuration (la camera bloquante est adapté pour ne pas bloquer avec l'extension terre ronde)
	FloorKind                     int
	FloorFile                     string
	RandomMapXSize                int  //Ajout d'une taille de map x pour la génération aléatoire
	RandomMapYSize                int  //Ajout d'une taille de map y pour la génération aléatoire
	TerreRonde                    bool //extension terre ronde
	PasMarcheEau                  bool //extension bloc d'eau bloqué
	BlocAnimated                  bool //extension pour l'animation du sol
	Zoom                          bool //extension permettant de zoomer et dezoomer (touche '+' pour dezoomer et '-' pour zoomer )
	Portal                        bool //extension qui permet la pose de portail ainsi que la teleportation (touche 't' pour créer un portail)
	Sauvegarde                    bool //extension permettant la sauvegarde du terrain aléatoire (touche 's' pour sauvegarder)
	Chasseautresor                bool //extension qui ajoute un objectif de chasse au tresor(voir doc dossier tresor)
